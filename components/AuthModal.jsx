...
export default function AuthModal({ isOpen, onClose, initialView = 'login' }) {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: '', companyName: '', taxId: '', 
    tcNo: '', licenseClass: '', plate: '' // <--- PLAKA EKLENDİ
  });
  ...
  const validateForm = () => {
    ...
    if (view === 'register') {
      ...
      if (userType === 'shipper') {
        ...
        if (!formData.plate || formData.plate.length < 5) { alert("Lütfen geçerli bir araç plakası giriniz."); return false; } // <--- PLAKA KONTROLÜ
      }
    }
    return true;
  };
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const userData = {
        ...
        role: userType,
        status: 'active', // <--- OTOMATİK AKTİF (Mobil uygulama için şart)
      ...
      if (userType === 'shipper') {
        ...
        // PLAKA VERİTABANINA YAZILIYOR (Bitişik ve Büyük Harf)
        userData.plate = formData.plate.toUpperCase().replace(/\s/g, ''); 
        userData.rating = "5.0";
      }
              ...
              <form onSubmit={handleRegister} className="space-y-3">
                ...
                {userType === 'shipper' ? (
                  ...
                  <>
                    ...
                    {/* PLAKA GİRİŞ ALANI */}
                    <div className="grid grid-cols-2 gap-3">
                        <Input label="Araç Plakası" name="plate" placeholder="34 TR 123" onChange={handleChange} />
                        <Input label="Telefon" name="phone" type="tel" onChange={handleChange} />
                    </div>
                  </>
                )}
  ...
  return (
    <div>
      ...
      <input 
        ...
        onChange={onChange} 
        className="w-full bg-[#0a192f] border border-slate-600 rounded-lg px-3 py-2 text-sm text-white focus:border-white outline-none" 
      />
    </div>
  )
}